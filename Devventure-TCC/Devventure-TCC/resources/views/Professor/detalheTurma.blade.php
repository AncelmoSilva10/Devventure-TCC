...
<body>
    ...
    <script>
        ...
        @if (session('aula_criada_feedback'))
            const feedback = @json(session('aula_criada_feedback'));
            Swal.fire({
                title: 'Aula Criada!',
        ...
        @elseif (session('formulario_criado_success'))
            Swal.fire({
                title: 'Tudo pronto!',
                text: "{{ session('formulario_criado_success') }}",
                icon: 'success',
                confirmButtonColor: '#00796b'
            });

        // 3. CENÁRIO: GENÉRICO (Convites, Edições, Criação de Turma, etc)
        @elseif (session('sweet_success'))
            Swal.fire({ 
                title: 'Sucesso!', 
        ...
        // 4. CENÁRIO: ERROS
        @elseif (session('sweet_error_convite'))
             Swal.fire({ 
                title: 'Atenção', 
                text: "{{ session('sweet_error_convite') }}", 
                icon: 'warning', 
